...
- **Cross-project analysis**: วิเคราะห์สถาปัตยกรรมข้าม Microservices ผ่านคำสั่ง `plan cross-project`
...
- **Harness tool validation**: ระบบตรวจสอบว่า Harness รองรับเครื่องมือที่ Role นั้นจำเป็นต้องใช้หรือไม่
- **Token optimization**: ระบบย่อความ 'Senior Technical Editor' (TH/EN) เพื่อสร้าง technical summary ที่มีคุณภาพสูงสำหรับทุกการสื่อสาร
- **CEO-to-agent gateway**: CEO สามารถสั่งการตรงถึง Agent ผ่านคำสั่ง `gateway` โดยข้าม trigger มาตรฐาน
- **Dhamma integration**: ฉีดหลักพุทธธรรม (Kalāma, Yoniso Manasikāra ฯลฯ) เข้าสู่ System Prompt ของ Agent เพื่อลด Hallucination
...
Skill มี file `VERSION` (semver, ปัจจุบัน `0.10.0`). ตอน update:
...
| `plan cross-project` | วิเคราะห์สถาปัตยกรรมข้าม Microservices                    | 2         |

### Phase 7 -- Sub-Agent Delegation

| Command     | คำอธิบาย                                                                | Risk Tier |
|-------------|------------------------------------------------------------------------|-----------|
| `delegate`  | ส่ง task ให้ agent. `--execute` รัน inline                            | 3         |
| `gateway`   | CEO ส่งคำสั่งตรงถึง Agent โดยเฉพาะ                                      | 2         |
